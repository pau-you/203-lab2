...
```{r}
#Improvement v1
poverty_model <- lm(cases_per_100k ~ `length_ shelter_in_place` + political_party_governor + percent_povertyline_2018 + white_percent_totalpop_cases, data=total)
...
poverty_model <- lm(cases_per_100k ~ `length_ shelter_in_place` + political_party_governor + percent_povertyline_2018 + white_percent_totalpop_cases + as.numeric(black_percent_totalpop_cases) + hispanic_percent_totalpop_cases + other_percent_totalpop_cases, data=total)
summary(poverty_model)
...
barplot(c(white, black, hispanic, other), main="Cases by Ethnicity",
        ...
        col = c(c1,c2,"green", "yellow"), ylim=c(0, 5000000))
```
