...
total <- read_csv("../data/interim/covid19+cusp.csv", col_names=TRUE)
total <- total %>% 
  rename(
...
Since we were moving the data from excel sheets to R we had to change the date format, and convert from LINUX. We found each column with dates and used the function "excel_numeric_to_date" to make these changes. 
...
```{r Plot Length Shelter in Place vs Cases per 100k}
...
total <- total %>% filter(white_percent_cases != "NR",
                 black_percent_cases != "NR",
                 hispanic_percent_cases != "NR",
                 other_percent_cases != "NR")
#x <- total[-c(12, 19, 32, 33, 35, 49), ]
```

```{r Baseline Model, tidy=TRUE}
...
```{r Number of Democratic vs Republican States}
...
```{r Cases in Republican and Democratic States}
...
```{r Improvement Model V1, tidy=TRUE}
...
```{r Improvement V2 Model, tidy=TRUE}
#Improvement v2
...
poverty_model_v2 <- lm(cases_per_100k ~ length_shelter_in_place + political_party_governor + 
                         ...
                         as.numeric(black_percent_cases) + as.numeric(hispanic_percent_cases) + as.numeric(other_percent_cases), data=x)
...
```{r Percent Cases by Ethnicity, warning = FALSE, tidy=TRUE}
...
```{r Linear Conditional Expectation Baseline Model}
...
```{r Plotting Linear Conditional Expectation Together Baseline Model}
...
```{r Linear Conditional Expectation Together Improvement Model 1}

model2_plot_1 <- x_resid %>%  
  ggplot(aes(x = length_shelter_in_place, y = poverty_model_resid)) + 
  geom_point() + stat_smooth()
...
```{r Plotting Linear Conditional Expectation Together Improvement Model V1}
...
```{r Linear Conditional Expectation Together Improvement Model V2}
model3_plot_1 <- x_resid %>%  
  ggplot(aes(x = length_shelter_in_place, y = poverty_model_v2_resid)) + 
  geom_point() + stat_smooth()
...
```{r Plotting Linear Conditional Expectation Together Improvement Model V2}
...
```{r Baseline Model Ocular Test}
plot_4
```

```{r Improvement Model V1 Ocular Test}
plot_5
```

```{r Improvement Model V2 Ocular Test}
plot_6
```
...
```{r Histogram of Baseline Model Residuals}
hist(x_resid$party_model_resid, main = "Histogram of Baseline Model Residuals")
```

```{r Q-Q Plot for Baseline Model Residuals}
...
```{r Histogram of Improvement(v1) Model Residuals}
hist(x_resid$poverty_model_resid, main = "Histogram of Improvement(v1) Model Residuals")
```

```{r Q-Q Plot for Improvement (v1) Model Residuals}
...
```{r Stargazer Table, warning=FALSE, tidy=TRUE}
